...
public class FridayManager {
	 ...
	 */
	protected int getNumActiveProcessors() {
	 ...
	 */
	protected int getNumActiveReaders() {
	...
	public void readChunk(long sequenceNumber) {
		...
		do {
			...
			else if (numRecsRead == -1) {
				//System.err.println( ((CDSSingleNameReader) dbReader).getId() + " is inactive, ignore it" );
				_inactive = ((CDSSingleNameReader) dbReader).getId();
			}
	...
	public void getRidOfDeadReaders() {
		...
		while (readerIterator.hasNext()) {
			...
			if (reader.isFinished()) readerIterator.remove();
		}
	}
	 ...
	 */
	protected void stopAllProcessors() {
